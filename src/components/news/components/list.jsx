...
export default function List() {
  ...
  const resolveImageUrl = (item) => {
    ...
    if (Array.isArray(imgs) && imgs.length > 0) {
      ...
      else if (first && (first.imageID || first.id)) imageID = first.imageID || first.id;
  ...
  return (
    <Row gutter={[16, 16]}>
      ...
      <NewsModal postId={selectedId} visible={modalVisible} onClose={closeModal} />
      ...
      <Col xs={24} sm={24} md={8}>
        <Card title="Tin nổi bật">
          <ul style={{ paddingLeft: 16 }}>
            {featured.map((it) => (
              <li key={it.id || it.postId || Math.random()} style={{ marginBottom: 8 }}>
                <a href={`#/news/${it.id || it.postId}`}>{it.title || it.name}</a>
              </li>
            ))}
          </ul>
        </Card>
